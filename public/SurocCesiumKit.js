/*!
 * cesium-suroc-kit v1.0.0
 * 一个由 Suroc 维护的轻量级 Cesium 快速开发工具集，封装了常用函数与操作，旨在提升 CesiumJS 引擎下的开发效率与代码简洁性
 * Copyright (c) Suroc.
 * This source code is licensed under the UNLICENSED license.
 */
/**
 * TokenManager.ts
...
class CesiumKitToken {
  ...
  async initialize() {
    try {
      ...
      try {
        // 初始化密钥
        this.MASTER_KEY = this.sodium.from_hex("00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff");
      }
      catch (keyError) {
  ...
  async generateToken(ip, fixed, expireTs) {
    ...
    const cipher = this.sodium.crypto_aead_xchacha20poly1305_ietf_encrypt(payload, null, null, nonce, this.MASTER_KEY);
    // 手动拼接 nonce 和密文
  ...
  async decodeToken(token) {
    ...
    try {
      ...
      let bytes;
      try {
        bytes = this.sodium.from_base64(token, this.sodium.base64_variants.URLSAFE_NO_PADDING);
      }
      catch (e) {
      ...
      let plain;
      try {
        plain = this.sodium.crypto_aead_xchacha20poly1305_ietf_decrypt(null, cipher, null, nonce, this.MASTER_KEY);
      }
      catch (e) {
...
class TokenManager {
  tokenInstance = null;
  isInitialized = false;
  FIXED_STRING = '^creatunion.aseem.SurocKit&';
  /**
   * 初始化 TokenManager
   * @returns 初始化是否成功
   */
...
// 导出单例实例
var SurocCesiumKitTokenManager = new TokenManager();