...
interface ElementsInstance {
  drop: () => void;
  clone: () => ElementsInstance;
}

interface ElementsStatic {
  fromTle: (sgp4Module: any, satName: string, line1: string, line2: string) => { tag: string; val?: any };
  new: (val: any, sgp4: any) => ElementsInstance;
}

// 常量类型定义
interface ConstantsInstance {
  drop: () => void;
  clone: () => ConstantsInstance;
  propagate: (minsFromEpoch: number) => { tag: string; val?: { position: number[]; velocity: number[] } };
}

interface ConstantsStatic {
  fromElements: (sgp4Module: any, elements: any) => { tag: string; val?: any };
  new: (val: any, sgp4: any) => ConstantsInstance;
}
...
const Object: {
  ...
  utf8_encode: (s: string, realloc: Function, memory: { buffer: ArrayBuffer }) => number;
...
class SurocSGP4 {
  ...
  private sgp4Module: any;
  private pointsCollection: any;
  private pointsCollectionPrimitives: any[];
  private pointsCollectionPrimitivesLen: number;
  private matchingIndexMetadata: Metadata[];
  ...
  private _movePolyline: any;
  private _polylineMap: Map<string, any>;
  private referenceFrame: any;
  private Elements: ElementsStatic;
  private Constants: ConstantsStatic;
  private unauthorized?: boolean;

  constructor(options: { token?: string; wasmurl: string; viewer?: any; pixelSize?: number; outlineWidth?: number; color?: any; model?: boolean; ModelSize?: number; ModelScale?: number; stepSeconds?: number; referenceFrame?: string }) {
    this.token = options.token || "";
    this.wasmurl = options.wasmurl;
    this.viewer = options.viewer || (window as any)['viewer'] || null;
  ...
  async init(TLEs: TLEData[]) {
    ...
    const promises = TLEs.map(async (TLE) => {
      ...
      if (!noradId || !/^\d+$/.test(noradId)) return;
      ...
      if (!sgp4Constants || sgp4Constants.tag !== 'ok') return;
   ...
   * @returns {any|null}
   */
  _targetOrbit(val: string | number | { id: string | number }, activeColor: any, type: string = 'CLICK', flag: boolean = false): any | null {
    if (!this.viewer || !this.matchingIndexMetadata.length) return null;
    ...
    ids.forEach(noradId => {
      ...
      if (entity) {
        if (flag) {
          ...
          const pointPrimitive = this.pointsCollection._pointPrimitives.find((p: any) => p.id == noradId);
          if (pointPrimitive) pointPrimitive.show = !entity.show;
        }
        orbitEntity = entity;
        return;
      }
      ...
      // 隐藏点
      const pointPrimitive = this.pointsCollection._pointPrimitives.find((p: any) => p.id == noradId);
      if (pointPrimitive) pointPrimitive.show = false;
   ...
   */
  _getPositionProperty(sgp4Constants: any, TLEEpochTs: number, totalSeconds: number): { positionProperty: any, velocityProperty: any } {
   ...
   */
  _deleteOrbit(ids?: string | number | Array<string | number>, type: string = "CLICK") {
  ...
  _animatePoints(): void {
    ...
    for (let i = 0; i < this.pointsCollectionPrimitivesLen; i++) {
      ...
      const pos = Cesium.Cartesian3.fromElements(...result.val.position.map((v: any) => v * 1000));
...
Object.clamp_guest = (i: number, min: number, max: number): number => {
  if (i < min || i > max) throw new RangeError(`must be between ${min} and ${max}`);
  return i;
}

Object.data_view = (mem: any, ...args: any[]): any => {
...
Object.to_string = (val: any): string => {
...
Object.utf8_encode = (s: string, realloc: Function, memory: { buffer: ArrayBuffer }): number => {
...
Object.utf8_encoded_len = (): number => {
  return UTF8_ENCODED_LEN;
}

class Slab {
  private list: Array<{ next: number, val: any }>;
  private head: number;
...
class Sgp4 {
  ...
  private _resource0_slab: Slab;
  private _resource1_slab: Slab;
  private _resource2_slab: Slab;
  private _registry0?: any;
  private _registry1?: any;
  private _registry2?: any;
  ...
  addToImports(imports: Record<string, any>): void {
    ...
    imports.canonical_abi['resource_drop_elements'] = (i: any) => {
      this._resource0_slab.remove(i).drop();
    };
    imports.canonical_abi['resource_clone_elements'] = (i: any) => {
      const obj = this._resource0_slab.get(i);
      return this._resource0_slab.insert(obj.clone())
    };
    imports.canonical_abi['resource_get_elements'] = (i: any) => {
      return this._resource0_slab.get(i)._wasm_val;
    };
    imports.canonical_abi['resource_new_elements'] = (i: any) => {
      const registry = this._registry0;
      return this._resource0_slab.insert(new Elements(i, this));
    };

    imports.canonical_abi['resource_drop_resonance-state'] = (i: any) => {
      this._resource1_slab.remove(i).drop();
    };
    imports.canonical_abi['resource_clone_resonance-state'] = (i: any) => {
      const obj = this._resource1_slab.get(i);
      return this._resource1_slab.insert(obj.clone())
    };
    imports.canonical_abi['resource_get_resonance-state'] = (i: any) => {
      return this._resource1_slab.get(i)._wasm_val;
    };
    imports.canonical_abi['resource_new_resonance-state'] = (i: any) => {
      const registry = this._registry1;
      return this._resource1_slab.insert(new ResonanceState(i, this));
    };

    imports.canonical_abi['resource_drop_constants'] = (i: any) => {
      this._resource2_slab.remove(i).drop();
    };
    imports.canonical_abi['resource_clone_constants'] = (i: any) => {
      const obj = this._resource2_slab.get(i);
      return this._resource2_slab.insert(obj.clone())
    };
    imports.canonical_abi['resource_get_constants'] = (i: any) => {
      return this._resource2_slab.get(i)._wasm_val;
    };
    imports.canonical_abi['resource_new_constants'] = (i: any) => {
      const registry = this._registry2;
      return this._resource2_slab.insert(new Constants(i, this));
    };
  }

  async instantiate(module: WebAssembly.Module | WebAssembly.Instance | ArrayBuffer | Uint8Array, imports?: Record<string, any>): Promise<void> {
    imports = imports || {};
    ...
    if (module instanceof WebAssembly.Instance) {
      ...
      const instance = result as unknown as WebAssembly.Instance;
  ...
  afspcEpochToSiderealTime(arg0: any) {
    const ret = this._exports!['afspc-epoch-to-sidereal-time'](+arg0);
    return ret;
  }
  iauEpochToSiderealTime(arg0: any) {
...
class Bindings {
  private _cache: Record<string, WebAssembly.Module>;
  private wasmurl: string;

  constructor(options: { wasmurl: string }) {
    this._cache = {}
    this.wasmurl = options.wasmurl
  }

  /** 缓慢地获取并编译WebAssembly模块 */
  async _getModule(filename: string): Promise<WebAssembly.Module> {
  ...
  async sgp4(options?: { imports?: Record<string, any> }): Promise<Sgp4> {
