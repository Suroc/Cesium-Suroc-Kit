 ...
 * @LastEditTime: 2025-08-19 10:52:52
 * @Description: 绘制工具
 */
...
export default class DrawTool {
  ...
  drawCircle() {
    ...
    this.handler.setInputAction((click) => {
      if (click && click.position) {
        ...
        this.drawObj = this.viewer.entities.add({
          position: new Cesium.CallbackProperty(() => new Cesium.Cartesian3.fromDegrees(...lngLat, 0), false),
          name: 'circle',
          id: id,
          ellipse: {
            width: this.config.borderWidth,
  ...
  */
  drawCirclePlane(radius, steps) {
    ...
    let lngLat = [];
    let polygon = [];
    const options = { steps: steps, units: "kilometers", properties: { foo: "bar" } };
  ...
  drawPlane() {
    ...
    this.handler.setInputAction(() => {
      ...
      try {
        if (this.handler && !this.handler.isDestroyed && typeof this.handler.destroy === 'function') {
          this.handler.destroy();
        }
      } catch (e) { }
