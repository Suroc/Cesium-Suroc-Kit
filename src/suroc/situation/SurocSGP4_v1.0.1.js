 ...
 * @LastEditTime: 2025-07-14 09:41:56
...
export default class SurocSGP4 {
    ...
    if (domain == decryptList[0]) {
      ...
      if (decryptList[2] == 'permanent' || myDate.getTime() <= endDate.getTime()) {
        ...
        const promises = TLEs.map(async (TLE) => {
          ...
          TLE.type = TLE.type ? TLE.type.replace(/\s+/g, '') : 'Unknown Type';
  ...
  _temeToJ2000Cached(temePosition, jd) {
    const key = jd.dayNumber + '-' + jd.secondsOfDay;
    if (!this._matrixCache) this._matrixCache = new Map();

    if (!this._matrixCache.has(key)) {
      ...
      this._matrixCache.set(key, fixedMat);
    }

    const matrix = this._matrixCache.get(key);
    return Cesium.Matrix3.multiplyByVector(matrix, temePosition, new Cesium.Cartesian3());
  }
   ...
   * @param {JulianDate} clickTime 当前点击或指向时间
   * @returns {Entity | null}
   */
  _targetOrbit(val, type, clickTime) {
    ...
    if (!this._orbitCache) this._orbitCache = new Map();
    ...
    ids.forEach(noradID => {
      ...
      const cached = this._orbitCache.get(noradID);
      if (cached && cached.lastTLEEpochTs === TLEEpochTs) {
        ({ positionProperty, velocityProperty } = cached);
      } else {
        ...
        this._orbitCache.set(noradID, {
          positionProperty,
          velocityProperty,
          lastTLEEpochTs: TLEEpochTs,
        });
      }
      // ---------- 🌟 缓存处理部分结束 ----------
  ...
  _deleteOrbit(ids, type = "CLICK") {
    const deleteById = (id) => {
      ...
      if (entity) {
        ...
        this._polylineMap.delete(`Orbit_${entity.id}`);
      }
    };
  ...
  */
  _clearOrbitCache(ids) {
    if (!ids) {
      this._orbitCache.clear();
      return;
    }

    if (typeof ids === 'string' || typeof ids === 'number') {
      this._orbitCache.delete(String(ids));
    } else if (Array.isArray(ids)) {
      ids.forEach(id => this._orbitCache.delete(String(id)));
    }
  }
